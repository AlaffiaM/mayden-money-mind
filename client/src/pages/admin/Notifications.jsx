...
export default function Notifications() {
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeading title="Notifications" subtitle="Compose in-app notifications and review what has been sent." />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <AdminCard title="Compose Notification">
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-gray-100 bg-mayden-gray/60 p-4">
              <p className="mb-2 text-xs uppercase tracking-wide text-gray-400">Preview</p>
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-100 bg-mayden-gray/60 px-5 py-4">
            <h2 className="font-serif font-bold text-mayden-dark">Notification History</h2>
          ...
          <div className="max-h-[600px] overflow-y-auto">
            {notifications.length === 0 ? (
              <p className="py-12 text-center text-gray-400">No notifications sent yet</p>
            ) : (
              <div className="divide-y divide-gray-100">
                {notifications.map((n) => (
                  <div key={n.id} className="group p-4 hover:bg-mayden-gray/40">
                    ...
                    <p className="mb-2 line-clamp-2 text-xs text-gray-500">{n.body}</p>
                    <div className="flex items-center gap-2">
                      {n.channels.split(",").map((ch) => (
                        <span key={ch} className="rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-gray-500">
                          {ch}
                        </span>
                      ))}
